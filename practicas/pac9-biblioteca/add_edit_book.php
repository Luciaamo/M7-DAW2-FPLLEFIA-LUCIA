...
$isEdit = isset($_GET['id']); // Si existe un parámetro `id`, es edición
$libros = $_SESSION['libros'] ?? []; // Obtener los libros guardados en la sesión
$libroActual = ['titulo' => '', 'autor' => '', 'imagen' => '', 'descripcion' => ''];

// Si es edición, buscar el libro con el ID especificado
if ($isEdit) {
...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    $id = $isEdit ? $_GET['id'] : uniqid(); // Reusar ID si es edición o generar uno nuevo
    ...
    // Actualizar libro existente o agregar uno nuevo
    if ($isEdit) {
        foreach ($libros as &$libro) {
            if ($libro['id'] == $id) {
                $libro = $nuevoLibro;
